...
void PairLJSmoothLinear::compute(int eflag, int vflag)
{
 ...
 std::string PathGraph = "./frozen_graph.pb";
...
//	tensorflow::Tensor Input1(tensorflow::DT_FLOAT, tensorflow::TensorShape({1,9}));
	...
	input_tensor_mapped(0, 0) = rsq;
	input_tensor_mapped(0, 1) = lj1[itype][jtype];
	input_tensor_mapped(0, 2) = lj2[itype][jtype];
	input_tensor_mapped(0, 3) = dljcut[itype][jtype];
	input_tensor_mapped(0, 4) = factor_lj;
	input_tensor_mapped(0, 5) = delx;
	input_tensor_mapped(0, 6) = dely;
	input_tensor_mapped(0, 7) = delz;
	input_tensor_mapped(0, 8) = cut[itype][jtype];
	...
	std::vector<std::pair<std::string, tensorflow::Tensor>> inputs = {
   	...
   	//	return 1;
        ...
        f[i][0]=double(output[0].matrix<float>()(0,0));
	f[i][1]=double(output[0].matrix<float>()(0,1));
	f[i][2]=double(output[0].matrix<float>()(0,2));
	f[j][0]=double(output[0].matrix<float>()(0,3));
	f[j][1]=double(output[0].matrix<float>()(0,4));
	f[j][2]=double(output[0].matrix<float>()(0,5));
	evdwl=double(output[0].matrix<float>()(0,6));
	session->Close();
	myfile  << f[i][0] <<","<< f[i][1] <<","<< f[i][2]  <<","<< f[j][0] <<","<< f[j][1] <<","<< f[j][2] << "," <<  evdwl  << "\n";
	/**
	inputs = {
   		 { "Input:0", Input1},
    	};
